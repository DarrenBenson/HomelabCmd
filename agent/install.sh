...
#
# Scope matches the agent's COMMAND_WHITELIST (defence in depth):
#   - apply_updates  -> apt-get update; apt-get upgrade -y
#   - restart_service -> systemctl restart <service>   (service name regex
#                        ^[a-zA-Z0-9_-]{1,64}$ enforced in agent code)
#   - clear_logs     -> journalctl --vacuum-time=7d
#
# Notes:
#  - apt-get install was removed: no agent action_type uses it, the wildcard
#    granted effectively-root via arbitrary package install.
#  - journalctl --vacuum-time is pinned to '7d' (the only value the agent
#    ever passes) instead of a wildcard.
#  - systemctl restart retains its wildcard because sudoers can't express the
#    agent's service-name regex; the agent code remains the constraining layer.

# apt-get for package management (no 'install' grant — unused by agent)
homelab-agent ALL=(root) NOPASSWD: /usr/bin/apt-get update
homelab-agent ALL=(root) NOPASSWD: /usr/bin/apt-get upgrade -y

# systemctl restart for service management (agent enforces service-name regex)
homelab-agent ALL=(root) NOPASSWD: /usr/bin/systemctl restart *

# journalctl vacuum for log management (pinned to 7d, matches agent constant)
homelab-agent ALL=(root) NOPASSWD: /usr/bin/journalctl --vacuum-time=7d
EOF
...
if [[ -n "$REGISTRATION_TOKEN" ]]; then
    ...
    if [[ "$AGENT_MODE" == "readwrite" ]]; then
        if [[ ! -f "$SUDOERS_FILE" ]]; then
            ...
            cat > "$SUDOERS_FILE" << 'EOF'
# HomelabCmd Agent - Passwordless sudo for whitelisted commands (BG0017)
# Scope matches the agent's COMMAND_WHITELIST. See full sudoers block above
# (top of install.sh, readwrite-mode branch) for the rationale comments.
homelab-agent ALL=(root) NOPASSWD: /usr/bin/apt-get update
homelab-agent ALL=(root) NOPASSWD: /usr/bin/apt-get upgrade -y
homelab-agent ALL=(root) NOPASSWD: /usr/bin/systemctl restart *
homelab-agent ALL=(root) NOPASSWD: /usr/bin/journalctl --vacuum-time=7d
